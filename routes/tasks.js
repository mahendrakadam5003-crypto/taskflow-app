...
router.get('/payment-history', async (req, res) => {
  ...
  let sql = `SELECT t.id, t.project_id, t.title, t.invoice_number, t.invoice_date, t.customer_name,
    ...
    WHERE (t.invoice_number IS NOT NULL AND trim(t.invoice_number) <> '') OR (t.customer_name IS NOT NULL AND trim(t.customer_name) <> '') OR (COALESCE(t.total_amount, 0) > 0)`;
...
router.get('/payment-history/summary', async (req, res) => {
  if (!(await canViewPaymentHistory(req))) return res.status(403).json({ error: 'You do not have payment-history access.' });
  const conditions = ["(t.invoice_number IS NOT NULL AND trim(t.invoice_number) <> '') OR (t.customer_name IS NOT NULL AND trim(t.customer_name) <> '') OR (COALESCE(t.total_amount, 0) > 0)"];
  const params = [];
