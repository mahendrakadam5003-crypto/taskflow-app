...
router.get('/payment-history', async (req, res) => {
  ...
  let sql = `SELECT t.id, t.project_id, t.title, t.invoice_number, t.invoice_date, t.customer_name,
    ...
    WHERE t.invoice_number IS NOT NULL AND trim(t.invoice_number) <> ''`;
  if (req.query.from) { sql += ' AND t.invoice_date >= ?'; params.push(req.query.from); }
  if (req.query.to) { sql += ' AND t.invoice_date <= ?'; params.push(req.query.to); }
  ...
  sql += ' ORDER BY t.invoice_date DESC, t.id DESC';
...
router.get('/payment-history/summary', async (req, res) => {
  if (!(await canViewPaymentHistory(req))) return res.status(403).json({ error: 'You do not have payment-history access.' });
  const conditions = ["t.invoice_number IS NOT NULL", "trim(t.invoice_number) <> ''"];
  const params = [];
  if (req.query.from || req.query.to) {
    conditions.push('t.invoice_date >= ?', 't.invoice_date <= ?');
    params.push(req.query.from || '0000-01-01', req.query.to || '9999-12-31');
  } else {
    conditions.push("t.invoice_date >= date('now', '-30 days')", "t.invoice_date <= date('now')");
  }
  if (req.query.assignee_id) { conditions.push('t.assignee_id = ?'); params.push(Number(req.query.assignee_id)); }
...
router.put('/payment-history/:id', async (req, res) => {
  ...
  const task = await db.prepare('SELECT id FROM tasks WHERE id=? AND invoice_number IS NOT NULL').get(req.params.id);
  ...
  await db.prepare('UPDATE tasks SET payment_status=?, payment_received_date=?, amount_received=?, updated_at=datetime(\'now\') WHERE id=?')
    .run(status, req.body.payment_received_date || null, received, req.params.id);
  res.json({ ok: true });
});
...
router.post('/projects/:id/tasks', requireProjectAccess, async (req, res) => {
  try {
    ...
    const info = await db.prepare(`INSERT INTO tasks(project_id,title,description,created_by,assignee_id,due_date,invoice_number,invoice_date,customer_name,total_amount)
      VALUES(?,?,?,?,?,?,?,?,?,?)`).run(req.params.id, title.trim(), String(description || '').trim(), req.session.userId, assignee_id || null, due_date || null, String(invoice_number || '').trim() || null, invoice_date || null, String(customer_name || '').trim(), Math.max(0, Number(total_amount) || 0));
