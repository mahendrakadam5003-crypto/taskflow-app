...
async function getLocationName(lat, lng) {
  try {
    // Queries OpenStreetMap's free reverse geocoding engine (Nominatim)
...
router.post('/punch-in', async (req, res) => {
  ...
  if (existing) {
    await db.prepare('UPDATE attendance SET punch_in = ?, in_lat = ?, in_lng = ?, location_status = ? WHERE id = ?')
      .run(now, lat, lng, mapStr, existing.id);
  } else {
    await db.prepare('INSERT INTO attendance (user_id, date, punch_in, in_lat, in_lng, location_status) VALUES (?, ?, ?, ?, ?, ?)')
...
router.post('/punch-out', async (req, res) => {
  ...
  await db.prepare('UPDATE attendance SET punch_out = ?, out_lat = ?, out_lng = ?, location_status = ? WHERE id = ?')
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    await db.prepare(`UPDATE attendance SET ${updates.join(', ')} WHERE id = ?`).run(...vals);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
