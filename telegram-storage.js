...
async function uploadToTelegram(file) {
  ...
  if (!document?.file_id || !response.data?.result?.message_id) throw new Error('Telegram did not return attachment details.');
  return { fileId: document.file_id, messageId: response.data.result.message_id };
}
