...
function openReimbursementDrawer(row) {
  ...
  $('#reimbursement-detail-receipt').innerHTML = receiptItems.length
    ? `<div class="receipt-gallery">${receiptItems.map((item, index) => String(item.mime_type || '').startsWith('image/')
      ...
      : `<a class="receipt-document-link" href="${item.url}" target="_blank" rel="noopener">${escapeHtml(item.original_name || 'View receipt document')}</a>`).join('')}</div>`
...
function startTaskListPolling() {
  stopTaskListPolling();
  taskListPollTimer = setInterval(async () => {
    if (document.hidden) return;
