    ...
    const renderRows = async () => {
      ...
      try {
        ...
        table.innerHTML = rows.length ? rows.map(row => `<tr>
          <td>${escapeHtml(row.assignee_name || 'Unassigned')}</td><td><b>${escapeHtml(row.invoice_number)}</b></td><td>${escapeHtml(row.invoice_date || '—')}</td><td>${escapeHtml(row.customer_name || '—')}</td>
