    ...
    const renderRows = async () => {
      ...
      try {
        ...
        table.innerHTML = rows.length ? rows.map(row => `<tr>
          <td><b>${escapeHtml(row.invoice_number)}</b></td><td>${escapeHtml(row.invoice_date || '—')}</td><td>${escapeHtml(row.customer_name || '—')}</td>
        ...
        </tr>`).join('') : '<tr><td colspan="10" class="hint" style="text-align:center;padding:15px;">No invoices found.</td></tr>';
        $$('.payment-save').forEach(button => button.onclick = async () => {
          const id = button.dataset.id;
          const received = Number($(`.payment-row-received[data-id="${id}"]`).value || 0);
          await api(`/payment-history/${id}`, { method: 'PUT', body: { payment_status: $(`.payment-row-status[data-id="${id}"]`).value, payment_received_date: $(`.payment-row-date[data-id="${id}"]`).value || null, amount_received: received } });
          await renderRows();
          showAppNotification('Payment history updated.');
        });
      } catch (error) { table.innerHTML = `<tr><td colspan="10" class="form-error">${escapeHtml(error.message)}</td></tr>`; }
    };
    $('#payment-history-filter').onclick = renderRows;
    renderRows();
}
